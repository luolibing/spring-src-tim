...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void initStrategies(ApplicationContext context) {
		...
		// 2 LocaleResolver国际化组件
		initLocaleResolver(context);
		// 3 ThemeResolver主题
		initThemeResolver(context);
		// 4 HandlerMappings映射
		initHandlerMappings(context);
		// 5 HandlerAdapters适配器
		initHandlerAdapters(context);
		// 6 HandlerExceptionResolvers异常解析
		initHandlerExceptionResolvers(context);
		// 7 RequestToViewNameTranslator请求到视图的解析器
	 ...
	 * 提供一个请求存储属性，可供其他请求使用，在使用重定向时非常必要
	 */
	private void initMultipartResolver(ApplicationContext context) {
		try {
			// 利用getBean进行初始化MultipartResolver
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		// 需要发现所有的HandlerMapping， 所有的HandlerMapping将组成一个链，然后逐个判断，遇到第一个可以处理的，直接返回
		if (this.detectAllHandlerMappings) {
			// Find all HandlerMappings in the ApplicationContext, including ancestor contexts.
			// 从上下文中获取到所有的HandlerMapping
			Map<String, HandlerMapping> matchingBeans =
					BeanFactoryUtils.beansOfTypeIncludingAncestors(context, HandlerMapping.class, true, false);
			if (!matchingBeans.isEmpty()) {
				this.handlerMappings = new ArrayList<HandlerMapping>(matchingBeans.values());
				// We keep HandlerMappings in sorted order. 如果有多个还需要进行排序
				OrderComparator.sort(this.handlerMappings);
			}
		}

		// 如果改为查询默认的，则直接超找handlerMapping的Bean
		else {
			try {
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				// 判断是否是Multipart类型， 如果是还需要将request封装为MultipartRequest
				processedRequest = checkMultipart(request);
				multipartRequestParsed = (processedRequest != request);

				// Determine handler for the current request.
				// 给request适配一个MappedHandler
